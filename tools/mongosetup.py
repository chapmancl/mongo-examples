...
# ---------------------------------------------------------------------------
MEMORY_DB_NAME = "ai_config"  # overridden by MEMORY_DB env var at runtime
...
def _get_settings_mongo_url(settings) -> str:
	mongo_url_value = getattr(settings, "mongo_uri", None)
...
def create_mcp_config_collections(settings) -> None:
	...
	#settings.mcp_config_db = "ai_config"
...
def create_mcp_cache_indexes(mongo_client: MongoDBClient) -> None:
	...
	"""
	collection = mongo_client.client[settings.mcp_config_db]["mcp_cache"]
	...
	if emb_idx not in existing:
		...
		print(f"Created index: {settings.mcp_config_db}.mcp_cache.{emb_idx}")
	else:
		print(f"Index already exists: {settings.mcp_config_db}.mcp_cache.{emb_idx}")
	...
	if entry_idx not in existing:
		...
		print(f"Created index: {settings.mcp_config_db}.mcp_cache.{entry_idx}")
	else:
		print(f"Index already exists: {settings.mcp_config_db}.mcp_cache.{entry_idx}")
	...
	if ttl_idx not in existing:
		...
		print(f"Created TTL index: {settings.mcp_config_db}.mcp_cache.{ttl_idx}")
	else:
		print(f"TTL index already exists: {settings.mcp_config_db}.mcp_cache.{ttl_idx}")
...
def main() -> None:
	...
	parser.add_argument(
		...
		action="store_true",
		default=False,
		help="Use lmig_settings.py credentials instead of local_settings.py",
	)
