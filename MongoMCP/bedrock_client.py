...
class BedrockClient:
    ...
    def __init__(self, settings):
        ...
        # Direct boto3 Bedrock client — retained for the embeddings path
        # (invoke_model); inference is routed via self.converse_client below.
        ...
        # Select the inference transport. LibertyAIR is the EA-sanctioned path
        # (Frameworks & SDKs Decision Record retires direct Bedrock); 'bedrock'
        # falls back to the boto3 client for a dev-only cutover escape hatch.
        ...
        if provider == "libertyair":
            if self.embedding_model_id.startswith("openai"):
                from .libertyair_embedding import LibertyAIREmbeddingClient
                self.embedding_client = LibertyAIREmbeddingClient.from_settings(self.settings)
            from .libertyair_converse import LibertyAIRConverseClient            
            self.converse_client = LibertyAIRConverseClient.from_settings(self.settings)
            # LibertyAIR's Bedrock route does not support cache points.
            self.enable_cache_points = False
            logger.info("LLM provider: LibertyAIR (model=%s)", self.settings.LLM_MODEL_ID)
        else:
    ...
    async def generate_embedding(self, text: str, model_id: Optional[str] = None) -> list:
        ...
        if model_id.startswith("voyage-"):
            logger.info(f"Generating embedding using Voyage model {model_id}")
            return await self.generate_voyage_embeddings(text, model_id=model_id)
        elif model_id.startswith("openai"):
            logger.info(f"Generating embedding using OpenAI model {model_id}")
            return await self.generate_openai_embeddings(text, model_id=model_id)
        else: #model_id.startswith("aws")
            logger.info(f"Generating embedding using Bedrock model {model_id}")
            return self.generate_bedrock_embeddings(text, model_id=model_id)
        
    async def generate_bedrock_embeddings(self, text: str, model_id: Optional[str] = None, is_query: bool = True) -> list:
    ...
    async def generate_voyage_embeddings(self, text: str, model_id: Optional[str] = None, is_query: bool = True) -> list:
        ...
        logger.info(f"GenerateEmbedding Using {input_type} embedding model: {model_id}")

        try:
            if isinstance(text, str):
                result = self.voyage_client.embed([text], model=model_id)
                return {
                    "embedding_model": model_id,
            ...
            logger.warning(f"Voyage Error: {e}")  
    ...
    async def generate_voyage_embeddings_batch(
        ...
        #TODO This method should no longer be used - generate embedding can take a sinlge or array of texts
        ...
        results = self.generate_voyage_embeddings(texts, model_id, is_query)
        return results
    ...
    async def generate_openai_embeddings(self, text: str, model_id: Optional[str] = None, is_query: bool = True) -> list:
        ...
        result = await asyncio.to_thread(self.embedding_client.get_embedding, text, model_id)
        return {
                    "embedding_model": model_id,
                    "vector": result[0]
                }
