...
export default function App() {
  ...
  function consumeStreamPayload(rawPayload) {
     //Debug BJBconsole.log('🔍 Raw payload:', rawPayload.substring(0, 200))
    try {
      let obj = parseMaybeJson(rawPayload)
       //Debug BJBconsole.log('📦 Parsed:', obj)
      ...
      if (obj.clear_history) {
        ...
        console.error('❌ Parse error:', e, 'Payload:', rawPayload)
  ...
  async function resetBackend() {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error || 'Failed to reset backend URL: ' + API_URL)
    } catch (e) { setError(String(e)) }
    finally {
      setMapData(null)
      setStreamedOutput('')
  ...
  return (
    <div className="chat-root">
      ...
      <header className="chat-header">
        <img src="/lm_logo2.jpg" alt="Logo" style={{ height: 44, width: 'auto' }} />
        <h1 style={{ margin: 0, fontSize: 20, fontWeight: 700, color: '#001E2B' }}>PILOT-DEMO GEM Exposure Viewer</h1>
