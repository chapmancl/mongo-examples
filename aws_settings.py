...
class AWSSettings:
    def __init__(self):
        ...
        self.TOOL_NAME = os.getenv('MCP_TOOL_NAME', 'gemSearch')
        self.IS_LOCAL = False
        ...
        # Platform-injected credentials (from camfile secrets block)
        self._credentials_cache: Dict[str, str] = {
            'username': os.getenv('DB_USERNAME', ''),
            'password': os.getenv('DB_PASSWORD', ''),
            'mongoUrl': os.getenv('DB_URI', os.getenv('MONGO_URI', '')),
        }

    def get_mongo_credentials(self) -> Dict[str, str]:
        """
        Return MongoDB credentials from platform-injected environment variables.
